 ...
 *
 * Description   : Classe qui contient des fonctions pour initialisé les ports, sockets et quelques attributs des
 * classes Clients et Serveur.
 *
 * Version       : 1.0
 *
 * Date          : 24/02/2022
...
public class InitParametre
{
     ...
     */
    public static int initMaxClients()
    {
        int maxClients = 0;
        int nbThreads = Runtime.getRuntime().availableProcessors() / 4; // ici on a choisit d'alouer 4 thread au mini
        // pour le calcule pur et dur, pas les ecoutes,et envoies.
        while (maxClients < 1 || maxClients > nbThreads)
        {
            System.out.println("Donner le nombre max de clients en simultané sachant que le serveur alloue 4 threads " +
                    "au minimum par clients, le nombre max de client est : " + nbThreads);
     ...
     */
    private static InetAddress initIp()
    {
